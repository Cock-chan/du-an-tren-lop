...
def dijkstra(level, start, goal, in_box=False, dead=False):
    rows = len(level)
    cols = len(level[0])
    dist = {start: 0}
    prev = {}
    ...
    while pq:
        ...
        for dx, dy in [(1,0), (-1,0), (0,1), (0,-1)]:
            ...
            if 0 <= nx < cols and 0 <= ny < rows:
                cell = level[ny][nx]
                if cell < 3 or (cell == 9 and (in_box or dead)):
                    nd = d + 1
...
# Blinky (đỏ) ở góc trên trái
blinky_x = 56
blinky_y = 58
blinky_direction = 0

# Inky (xanh) ở gần cửa ra
inky_x = 450
inky_y = 350
inky_direction = 2

# Pinky (hồng) ở góc trên phải
pinky_x = 840
pinky_y = 58
pinky_direction = 2

# Clyde (cam) ở góc dưới trái
clyde_x = 56
clyde_y = 800
clyde_direction = 2
...
class Ghost:
    ...
    def move_with_dijkstra(self):
        num1 = (HEIGHT - 50) // 32
        num2 = WIDTH // 30

        if self.dead:
            goal = (14, 15)   # toạ độ cửa nhà
        else:
            goal = (self.target[0] // num2, self.target[1] // num1)

        start = (self.center_x // num2, self.center_y // num1)
        path = dijkstra(level, start, goal, self.in_box, self.dead)
        ...
        if path:
            ...
            if dx == 1: self.direction = 0; self.x_pos += self.speed
            elif dx == -1: self.direction = 1; self.x_pos -= self.speed
            elif dy == -1: self.direction = 2; self.y_pos -= self.speed
            elif dy == 1: self.direction = 3; self.y_pos += self.speed
        ...
        if self.dead and (goal[0] == start[0] and goal[1] == start[1]):
            self.dead = False
            self.in_box = False
...
while run:
    ...
    if moving:
        ...
        blinky.target = (player_x, player_y)
        pinky.target = (player_x, player_y)
        inky.target = (player_x, player_y)
        clyde.target = (player_x, player_y)


        if not blinky_dead and not blinky.in_box:
            blinky.target = (player_x, player_y)
            blinky_x, blinky_y, blinky_direction = blinky.move_with_dijkstra()
        if not pinky_dead and not pinky.in_box: 
            pinky.target = (player_x, player_y) 
            pinky_x, pinky_y, pinky_direction = pinky.move_with_dijkstra() 
        if not inky_dead and not inky.in_box: 
            inky.target = (player_x, player_y) 
            inky_x, inky_y, inky_direction = inky.move_with_dijkstra() 
        if not clyde_dead and not clyde.in_box:
            clyde.target = (player_x, player_y) 
            clyde_x, clyde_y, clyde_direction = clyde.move_with_dijkstra()
        clyde_x, clyde_y, clyde_direction = clyde.move_with_dijkstra()
